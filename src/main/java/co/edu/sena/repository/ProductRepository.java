...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(
        value = "select distinct product from Product product left join fetch product.provider left join fetch product.presentation",
        countQuery = "select count(distinct product) from Product product"
    )
    Page<Product> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct product from Product product left join fetch product.provider left join fetch product.presentation")
    List<Product> findAllWithToOneRelationships();

    @Query(
        "select product from Product product left join fetch product.provider left join fetch product.presentation where product.id =:id"
    )
